...
export default class BetPool {
    ...
    transferFunds(token: string, from: string, to: string, amount: bigint) {
        try {
            if (token === "0x0000000000000000000000000000000000000000") {
                this.wallet.transferEther(from, to, amount);
            } else {
                this.wallet.transferERC20(getAddress(token), from, to, amount);
            }
        } catch (error) {
    ...
    close() {
        if (this.fundsLocked > BigInt(0)) {
            try {
                this.transferFunds(this.tokenAddress, this.poolAddress, DAO_ADDRESS, this.fundsLocked);
